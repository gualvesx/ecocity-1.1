...
export const MyMapPointRequests = () => {
  const { 
    mapPointRequests, 
    isLoading, 
    error,
    ...
    return (
      <div className="p-8 text-center">
        <p className="text-lg text-muted-foreground">
          Você precisa estar logado para ver suas solicitações.
        </p>
      </div>
    );
  }
  ...
  if (mapPointRequests.length === 0) {
    return (
      <div className="p-8 text-center">
        <p className="text-lg text-muted-foreground">
          Você ainda não tem nenhuma solicitação de ponto ecológico.
        </p>
      </div>
    );
  }
