...
const Login = () => {
  ...
  const { execute: executeCaptcha, loading: captchaLoading, verified: captchaVerified } = useCaptcha({
    action: 'login',
    expectedScore: 0.5
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log("Executando reCAPTCHA para login...");
      ...
      console.log(`reCAPTCHA verificado pelo Firebase App Check`);
      console.log("Attempting login with:", email);
      
      const success = await login(email, password, captchaResult.token);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      console.log("Executando reCAPTCHA para Google login...");
      const captchaResult = await executeCaptcha('google_login');
            ...
            {view === 'login' && (
              <>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  >
                    {isLoading || localLoading || captchaLoading ? 'Verificando...' : 'Entrar'}
                ...
                <div className="mt-4 text-xs text-center text-muted-foreground">
                  Este site é protegido pelo reCAPTCHA Enterprise com verificação no servidor
                </div>
              </>
            )}
