    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        try {
          ...
          console.log(`Processando ponto: ${data.Nome}`);
          ...
          // Create map point - mapping CSV fields to database fields correctly
          const result = await addMapPoint({
            name: data.Nome, // Nome -> name
            type: pointType,
            description: data.Descrição || 'Ponto importado do CSV', // Descrição -> description
            address: correctedAddress, // Endereço -> address
            impact: data.Impacto || 'Impacto ambiental positivo', // Impacto -> impact
            contact: data.Contato || '', // Contato -> contact
            website: data.WebSite || '', // WebSite -> website
            lat: coordinates.lat,
            lng: coordinates.lng,
            openingHours: '',
            weeklyHours: {}
          ...
          if (result) {
            successCount++;
            console.log(`Ponto criado: ${data.Nome}`);
          } else {
            errors.push(`Linha ${i + 1}: Falha ao criar ponto: ${data.Nome}`);
            errorCount++;
          }
      ...
      // Show results
      if (successCount > 0) {
        toast.success(`${successCount} pontos importados com sucesso!`);
      }
